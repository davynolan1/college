...
{ int runnableFound; // DO NOT MODIFY/DELETE
  ...
  int * runCount = calloc(NPROC, sizeof(unsigned int)); //count for runs, initialise to 0's
  ...
  while(runnableFound){ // DO NOT MODIFY
    ...
    for(int i = 0; i < NPROC; i++){
      p = &ptable.proc[(lowRun+i)%NPROC];

      if(p->state != RUNNABLE){
        continue;
	    }
      
      if((runCount[p->pid] < runCount[tmp]) || (&ptable.proc[tmp])->state != RUNNABLE){
        tmp = p->pid;
	    }
    }
    lowRun = tmp;

    p = &ptable.proc[lowRun];
	  if(p->state != RUNNABLE){
		  continue;
	  }
	  runCount[lowRun]++;
