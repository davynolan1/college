...
{ int runnableFound; // DO NOT MODIFY/DELETE
  ...
  int runTable[NPROC]; // table for counting number of runs
  //initialise it to 0's
  for(int i=0; i < NPROC; i++){
	  runTable[i] = 0;
  }
  
  int lowestRunnable = -1;
  ...
  while(runnableFound){ // DO NOT MODIFY
    ...
    int tmpLowestRunnable = lowestRunnable;
    for(int i = 0; i < NPROC; i++){
      p = &ptable.proc[(lowestRunnable+i)%NPROC];

      if(p->state != RUNNABLE){
        continue;
	  }
      
      if((runTable[p->pid] < runTable[tmpLowestRunnable]) || (&ptable.proc[tmpLowestRunnable])->state != RUNNABLE){
        tmpLowestRunnable = p->pid;
	  }
    }
    lowestRunnable = tmpLowestRunnable;
