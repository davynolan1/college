...
float ***new_empty_3d_matrix(int dim0, int dim1, int dim2)
{
  ...
  mat3d = malloc(dim0 * sizeof(float ***));
  mat2d = malloc(dim0 * dim1 * sizeof(float **));
  mat1d = malloc(dim0 * dim1 * dim2 * sizeof(float *));

  assert((mat3d != NULL) || (mat2d != NULL) || (mat1d != NULL));

  int i, j, k;
  for (i = 0; i < dim0; i++)
  {
    mat3d[i] = &(mat2d[i * dim1]);
    for (j = 0; j < dim1; j++)
    {
      mat3d[i][j] = &(mat1d[i * dim1 * dim2 + j * dim2]);
    }
  }
  return mat3d;
}
