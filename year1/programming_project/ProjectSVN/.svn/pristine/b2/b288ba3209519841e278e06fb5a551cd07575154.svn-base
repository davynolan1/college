class Reviewer {
  ...
  void draw() {
    ...
    if (reviewPrintIndex < userReviewSet.size()) {
      ...
      if(tempBusName.equals("#N/A")){
        text("Business: "+tempBusName, 400, 200+scrollPos);
      }
      else{
        text("Business: "+tempBusName.substring(3, tempBusName.length()-3), 400, 200+scrollPos);
      }
      String tempDate = userReviewSet.get(reviewPrintIndex).date;
      text("Date: "+tempDate, 400, 250+scrollPos);

      text("REACTIONS", 140, 500+scrollPos);
      int tempUseful = userReviewSet.get(reviewPrintIndex).useful;
      //text("Useful: "+tempUseful, 75, 400+scrollPos);
      int tempFunny = userReviewSet.get(reviewPrintIndex).funny;
      //text("Funny: "+tempFunny, 75, 450+scrollPos);
      int tempCool = userReviewSet.get(reviewPrintIndex).cool;
      //text("Cool: "+tempCool, 75, 500+scrollPos);
      int numOfStars = userReviewSet.get(reviewPrintIndex).numOfStars;
